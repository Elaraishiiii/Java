...
public class Frazione {
    ...
    public static Frazione parseFrazione (String s) {
        Frazione ris;
        try {
            ris = new Frazione (Integer.parseInt(s),1);
        } catch (NumberFormatException E1) {
            int div = s.indexOf('/');
            try {
                String sn = s.substring(0,div);
                String sd = s.substring(div+1, s.length());
                ris = new Frazione (Integer.parseInt(sn), Integer.parseInt(sd));
